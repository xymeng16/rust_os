...
impl LinkedListAllocator {
    ...
    /// Look for a free region with the given size and alignment and remove
    /// it from the list.
    ///
    ...
    fn find_region(&mut self, size: usize, align: usize) -> Option<(&'static mut ListNode, usize)> {
        ...
        while let Some(ref mut region) = current.next {
            // check if current.next is suitable for the given size and alignment
            if let Ok(alloc_start) = Self::alloc_from_region(&region, size, align) {
                // region is suitable for the size and align, hence removing it from the list
    ...
    fn alloc_from_region(region: &ListNode, size: usize, align: usize) -> Result<usize, ()> {
        ...
        if remaining_size > 0 && remaining_size < mem::size_of::<ListNode>() {
            // the remaining region should be able to hold at least one ListNode
            return Err(());
        }
...
unsafe impl GlobalAlloc for Locked<LinkedListAllocator> {
    ...
    unsafe fn dealloc(&self, ptr: *mut u8, layout: Layout) {
        // drawback: smaller blocks will never be merged
